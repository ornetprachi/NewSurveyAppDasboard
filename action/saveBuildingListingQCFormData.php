...
if ($_SERVER['REQUEST_METHOD'] === "POST") {
    ...
    if (
        ...
        (isset($_POST['DeleteBldPltimage']) && !empty($_POST['DeleteBldPltimage']))
    ) {

        $Society_Cd = $_POST['Society_Cd'];
        $SiteName = $_POST['SiteName'];
        $Delete = $_POST['DeleteBldPltimage'];

        if($Delete == 'BuildingPlate'){
        $Query = " UPDATE Society_Master
                    SET Building_Plate_Image  = ''
                    WHERE SiteName ='$SiteName' AND Society_Cd = '$Society_Cd'";
        }else if($Delete == 'Building'){
            $Query = " UPDATE Society_Master
            SET Building_Image  = ''
            WHERE SiteName ='$SiteName' AND Society_Cd = '$Society_Cd'";
        }
        // print_r($Query);
        $SocietyMasterDeleteBldImg = $db->RunQueryData($ULB,$Query, $userName, $appName, $developmentMode);
        if($SocietyMasterDeleteBldImg == true){
            echo json_encode(array('statusCode' => 200, 'msg' => "Deleted!"));
        }
        else{
            echo json_encode(array('statusCode' => 204, 'msg' => 'Error occured, please try again' , 'error' => $SocietyMasterDeleteBldImg));
        }
    }
    ...
    ) 
    {
        ...
        if((isset($_POST['societyMar']) && !empty($_POST['societyMar']))){
            $societyMar = "N'".trim($_POST['societyMar'])."'";
        }else{
            $societyMar = "NULL";
        }
        ...
        if(isset($_FILES['buildingImg']['name'])){
            ...
            $target_path = $target_path . "BP_" . $Society_Cd.".jpg" ;
        ...
        if(isset($_FILES['buildingPlateImg']['name'])){
            ...
            // $target_filename = round(microtime(true)) .'_'. $electionName .'_'. $pocket_Cd . '.' . end($temp);
            $target_path = $target_path . "BPI_" . $Society_Cd.".jpg" ;
        ...
        if(!empty($DBName)){
            ...
            $sql1 = "UPDATE [Survey_Entry_Data].[dbo].[Society_Master] SET
                Site_Cd = '$Site_Cd',
                SiteName = '$SiteName',
                SocietyName = '$society',
            ...
            if($SocietyMasterBListQCUpdate == true){
                ...
                $sql2 = "UPDATE $DBName.[dbo].[SubLocationMaster] SET
                    Site_Cd = '$Site_Cd',
                    SiteName = '$SiteName',
                    SocietyName = '$society',
