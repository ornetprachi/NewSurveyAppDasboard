...
if ($_SERVER['REQUEST_METHOD'] === "POST") {
    ...
    //==================================================Delete Extra LOCKROOM Start==============================================================================================
    if(
        (isset($_POST['Sublocation_Cd']) && !empty($_POST['Sublocation_Cd'])) &&
        (isset($_POST['RoomNo']) && !empty($_POST['RoomNo'])) &&
        (isset($_POST['DeleteExtraLockRoom']) && $_POST['DeleteExtraLockRoom'] == 'DeleteExtraLockRoom')
    ) 
    {
        $DBName = '';
        if(
        ...
        if(!empty($DBName)){
            ...
            $sql3 = "DELETE FROM $DBName.[dbo].[LockRoom]
            WHERE Sublocation_Cd = $Sublocation_Cd AND RoomNo = '$RoomNo';";

            $ExtraLockRoomDelete = $db->RunQueryData($ULB,$sql3, $userName, $appName, $developmentMode);

            if($ExtraLockRoomDelete){
                ...
                echo json_encode(array('statusCode' => 204, 'msg' => 'Error occured! please try again later'));
            }
        }
        else{
            echo json_encode(array('statusCode' => 204, 'msg' => 'Couldnot fetch DataBase Name, Error occured! please try again later'));
        }
    ...
    ) 
    {
        ...
        $sql1 = "INSERT INTO $DBName.[dbo].[NewVoterRegistrationDeleted] (Voter_Cd, Name, NameM, Middlename, MiddlenameM
        ...
        SELECT Voter_Cd, Name, NameM, Middlename, MiddlenameM
            ...
            ,ControlChartWard, LockedButSurvey, HouseStatus_Cd, Hstatus, OwnerMobileNo, District, Ac_No, Col1, Col2, Col3, Col4, Col5, MarNmar, Religion, SubCaste
            ,AndroidFormNo, FamilyNo, AnniversaryDate, StayOutside, FamilyCount, VidhanSabha, Site_Cd, SiteName, DSK_UpdatedByUser, DSK_UpdatedDate
            ,FORMAT(SYSUTCDATETIME() AT TIME ZONE 'UTC' AT TIME ZONE 'India Standard Time', 'yyyy-MM-dd HH:mm:ss.fff'), '$userName', 1 , Survey_Society_Cd, Voter_Id, List_No, MajorIssues, Bpl, BloodGroup, FavParty, ShiftedStatus_Cd
            ,Sstatus, Dead, VMark_Cd, VMarkName, Email, Mobile, VoterDone, SF, Srno, Leader_Cd, Leader, Party_Cd, Party, Remark2, Remark3, Remark4
            ,SubLocationNo, SocietyNameM, NoOfVisits, PhoneNo, Longitude, Latitude, Altitude, Nmar, Nmar_Det, AddedDate, BoothName, BoothNameMar
            ,BoothRoomNo, BootKendraNo, Comitee_Cd, Comitee, City_Cd, City, NewVoterFlag, Recieved_Status, Category_Cd, QC_Calling_UpdatedDate
            ,QC_Calling_Status_Cd, SMS_UpdatedDate, SMS_Flag, QC_Calling_UpdatedByUser, QC_Birthday_Wish_updatedDate, QC_Birthday_Wish_Status
            ,QC_Birthday_Wish_UpdatedByUser, SurveyDate_2018, QC_Calling_Recording, QC_Birthday_Recording, QC_Remark, QC_Round1, QC_Round2, QC_Round3, Font_Cd
            ,BD_Call_Executive_Cd, BD_Call_Temp_Exe_Cd, SR_Call_Executive_Cd, SR_Call_Temp_Exe_Cd, BD_Call_Assign_Date, SR_Call_Assign_Date, NewRoomNo
        FROM $DBName.[dbo].[NewVoterRegistration]
        ...
        if($NewVotRegDeletedAdd){
            $sql2 = "DELETE FROM $DBName.[dbo].[NewVoterRegistration]
            WHERE Voter_Cd = $NonvVoterVoter_Cd;";
    ...
    ) 
    {
        ...
        if(!empty($DBName)){
            ...
            $sql = "SELECT 
                ...
                COALESCE(CONVERT(VARCHAR,CONVERT(DATE,BirthDate,23),101), '') AS BirthDate,
                --COALESCE(CONVERT(VARCHAR,BirthDate,23), '') AS BirthDate,
                ...
                COALESCE(CONVERT(varchar,UpdatedDate,120), '') AS UpdatedDate,
            ...
            FROM $DBName.[dbo].[NewVoterRegistration] Where Voter_Cd = $NonvVoterVoter_Cd";
            ...
            if(sizeof($GetNonVoterData)){
                ...
                if(!empty($Birthdate)){
                    $BirthDateCond = "BirthDate='$Birthdate',";
                }else{
                    $BirthDateCond = "";
                }
                ...
                if($UpdateByUser != "" || $UpdateByUser != NULL){
            ...
            $sql = "UPDATE $DBName.[dbo].[DW_VotersInfo] SET 
                        UpdatedStatus = 'Y',
                        HStatus = '$Hstatus',
                        SStatus = '$Sstatus',
                        $ReligionCond
                        ...
                        $UpdatedDateCond
                        $UpdateByUserCond
                        QC_UpdatedDate=FORMAT(SYSUTCDATETIME() AT TIME ZONE 'UTC' AT TIME ZONE 'India Standard Time', 'yyyy-MM-dd HH:mm:ss.fff'),
    ...
    // --------------FAMILY UPDATE------------------------------------------------------------------------
            ...
            if($DwVotersInfoUpdate == true){
                ...
                if($NewVotRegDeletedAdd){
                    $sql2 = "DELETE FROM $DBName.[dbo].[NewVoterRegistration] WHERE Voter_Cd = $NonvVoterVoter_Cd;";
