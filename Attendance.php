...
<section id="dashboard-analytics">
    ...
    $dataSite = array();
        $dataSite = $db->getSiteDropDownDatabyElectionNameAttendance($userName, $appName,  $developmentMode, $electionCd, $electionName);
    ...
    if(isset($_SESSION['SurveyUA_designation_Attendanmce']) && !empty($_SESSION['SurveyUA_designation_Attendanmce'])){
     ...
     $TableQuery = "SELECT 
                    ...
                    INNER JOIN [$ServerIP].[Survey_Entry_Data].[dbo].[Executive_Master] as em on (ed.Executive_Cd = em.Executive_Cd) 
                    INNER JOIN [$ServerIP].[Survey_Entry_Data].[dbo].[Site_Master] as sm on (ed.SiteName = sm.SiteName) 
                    INNER JOIN [$ServerIP].[Survey_Entry_Data].[dbo].[Executive_Master] as em1 on (sm.SupervisorName = em1.ExecutiveName)
                    LEFT JOIN [$ServerIP].[Survey_Entry_Data].[dbo].[Election_Master] as elm on (ed.ElectionName = elm.ElectionName) 
                    WHERE CONVERT(varchar,ed.SurveyDate,23) = '$Date' AND em.EmpStatus = 'A'
                    $SiteNameCondition $FilterTypeCondition $DesignationCon
    ...
    if(isset($_SESSION['SurveyUA_Attedance_Tab']) && !empty($_SESSION['SurveyUA_Attedance_Tab'])){
        $AttendaceTab = $_SESSION['SurveyUA_Attedance_Tab'];
    }else{
        $AttendaceTab = "";
    }
                ...
                <div class="content-body">
                    <div class="card-content">
                        <div class="card-body" style="margin-top: -12px;margin-bottom:-30px;">
                            <div class="row" <?php if(isset($_GET['p']) && $_GET['p'] != 'AttendanceModule'){ echo "style='background-color: #F8F8F8;border-radius: 10px;'"; } ?>>
...
<div class="tab-pane" id="home" aria-labelledby="home-tab" role="tabpanel" style="margin-top: -25px;">
    <ul class="nav nav-tabs" role="tablist" style="margin-left:8px;">
        <li class="nav-item">
            <a class="nav-link <?php if(empty($AttendaceTab)){ echo "active"; } ?>" id="Attendance-tab" data-toggle="tab" href="#Attendance" aria-controls="Attendance" role="tab" aria-selected="flase">Attendance</a>
        </li>
        <li class="nav-item ">
            <a class="nav-link <?php if(!empty($AttendaceTab) && $AttendaceTab == "AttedanceReportScreen"){ echo "active"; }?>" id="AttendanceReport-tab" data-toggle="tab" href="#AttendanceReport" aria-controls="AttendanceReport" role="tab" aria-selected="true">Report</a>
        </li>
    </ul>
    <div class="tab-content">
        <div class="tab-pane  <?php if(empty($AttendaceTab)){ echo "active"; } ?>" id="Attendance" aria-labelledby="Attendance-tab" role="tabpanel">
            <div class="content-body">
                                                                    ...
                                                                    <tbody>
                                                                        <?php
                                                                            if(sizeof($TableData) > 0){
                                                                                ...
                                                                                <tr>
                                                                                    <td>
                                                                                        <input class="form-check-input element checkbox" type="checkbox" style=" margin-left:10px;margin-bottom:10px;width: 15px; height: 15px;margin-top:-8px;" value="<?php echo $value['Executive_Cd'] ; ?>" id="AssignCheckbox" onclick="getSelectedExecutivesList(<?php echo $value['Executive_Cd'] ; ?>)" >
                                                                                    </td>
                                                                                    <!-- <td><?php //echo $srNo++; ?></td> -->
        ...
        <div class="tab-pane <?php if(!empty($AttendaceTab) && $AttendaceTab == "AttedanceReportScreen"){ echo "active"; }?>" id="AttendanceReport" aria-labelledby="AttendanceReport-tab" role="tabpanel">
            <div class="content-body">
                <section id="basic-datatable">
                    <?php include 'setAttedanceReportData.php'; ?>
                </section>
            </div>
        </div>
    </div>
...
<?php
unset($_SESSION['SurveyUA_Attedance_Tab']);
?>
    ...
    <script>
        ...
        // New Selection Code ----------------------------------
            ...
            function getSelectedExecutivesList(Executive_Cd){
                var selected = 0;
